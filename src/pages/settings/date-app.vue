    ...
    <AdminLayout>
        <template #main>
            <v-container class="flex">
                <v-row>
                    <v-col cols="12" md="4">
                        <AwtrixDateAppStyle :settingInfo="settingsInfo"
                            @onDateAppTimeFormatChanged="onDateAppTimeFormatChangedEvent"
...
async function onDateAppTimeFormatChangedEvent(format: string) {
    ...
    try {
        await awtrixClinet.value.setDateFormat(format);
    } catch (e) {
...
async function onAppTextColorChangedEvent(color: string) {
    if (!awtrixClinet.value) return;
    try {
        await awtrixClinet.value.setDateAppColor(color);
    } catch (e) {
        notification.push("Awtrix connection error", 'error', intervalTime);
    }
}
